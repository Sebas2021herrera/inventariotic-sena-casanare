...
@section('content')
@php use App\Models\EquipoEnergia; @endphp

<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-black text-gray-800 uppercase italic tracking-tighter">
                Nuevo Equipo <span class="text-[#39A900]">Energía</span>
            </h1>
            <p class="text-gray-500 text-sm font-bold italic">UPS · Regulador · Planta · Tablero</p>
        </div>
    ...
    <form action="{{ route('equipos-energia.store') }}" method="POST" class="space-y-6" id="form-energia">
        @csrf
        @include('equipos_energia._form', ['equipo' => null])
        <div class="flex gap-3 justify-end pt-2">
            <a href="{{ route('equipos-energia.index') }}" class="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-2xl font-black text-xs uppercase tracking-widest transition">Cancelar</a>
            <button type="submit" class="px-8 py-3 bg-[#39A900] hover:bg-green-700 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg transition">
                <i class="fas fa-save mr-2"></i> Registrar Equipo
            </button>
        </div>
    </form>
</div>

...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border-b-4 border-purple-500">
        <div class="flex items-center">
            ...
            <div>
                <p class="text-sm text-gray-500 uppercase font-bold">Sedes</p>
                <h3 class="text-2xl font-black">{{ $stats['sedes'] ?? 0 }}</h3>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        ...
        <form action="{{ route('dispositivos.importar') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="file" name="archivo" class="w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 cursor-pointer border border-dashed border-gray-200 p-2 rounded-xl" required>
            </div>
        ...
        @if(session('success'))
            <div class="mt-4 p-3 bg-green-100 text-green-700 rounded-xl text-xs flex items-center border border-green-200">
                ...
                <div>
                    <span class="font-bold">¡Éxito!</span><br>
                    {{ session('success') }}
                </div>
            </div>
        @endif
        ...
        @if($errors->any())
            <div class="mt-4 p-3 bg-red-50 text-red-700 rounded-xl text-xs border border-red-100">
                <div class="flex items-center mb-1 font-bold">
                    <i class="fas fa-times-circle mr-2 text-red-500"></i> Errores encontrados:
                </div>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-700">{{ $dispositivo->marca }}</div>
                        ...
                        <td class="px-6 py-4">
                         ...
                         <td class="px-6 py-4">
                                            @php
                                                // Normalizamos el texto: quitamos espacios y pasamos a mayúsculas
                                                $estado = strtoupper(trim($dispositivo->estado_fisico));
                                            @endphp

                                            <span class="px-2 py-1 text-[10px] font-black uppercase rounded-full {{ $estado == 'BUENO' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center items-center space-x-4">
                                ...
                                class="text-blue-500 hover:text-blue-700 transition transform hover:scale-120" 
                                ...
                                <form action="{{ route('dispositivos.destroy', $dispositivo) }}" 
                                    method="POST" 
                                    class="inline-block"
                                    onsubmit="return confirm('¿Está seguro de eliminar la placa {{ $dispositivo->placa }}? Esta acción es irreversible.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="text-red-500 hover:text-red-700 transition transform hover:scale-120" 
